...
\Cref{sec:order-type} then briefly introduces a scale of asymptotic growth for the modulus of entire functions, in the form of order $\rho \in [0, \infty]$ and type $\tau \in [0, \infty]$. Roughly speaking, if $f$ is of finite order, then $f$ grows not faster than a function of the form $e^{Q(z)}$, where $Q$ is a polynomial, as $\vert z \vert \to \infty$. Results introduced here will frequently be utilized towards the end of \Cref{ch:growth-zeros-entire-functions}.
