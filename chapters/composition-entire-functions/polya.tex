...
\begin{remark}    
    To justify our next definition, let $G \subset \C$ be a bounded domain with $0 \in G$. We claim that $\cl{G}$ contains a circle (the boundary of a disk) centered at $0$ of maximal, non-zero radius; in other words, we claim that the set
    $$ S \coloneqq \{ r > 0 : \partial B_r(0) \subseteq \cl{G} \} $$
    has a maximum. Firstly, since $G$ is open and contains $0$, there is some $t > 0$ such that $B_t(0) \subseteq G$. Since $\partial B_t(0) \subset \cl{B_t(0)} \subseteq \cl{G}$ we thus have $t \in S$ and $S$ is non-empty. It now suffices to show that the set $S_0 \coloneqq S \cup \{ 0 \}$ is compact. Clearly $S_0$ is bounded from below by $0$ and since $\cl{G}$ is bounded it must also be bounded from above. Thus it remains to show that $S_0$ is closed.

    Let $(r_n)_{n \in \N}$ be a convergent sequence in $S_0$ with limit $r \in [0, \infty)$. If $r = 0$, then $r \in S_0$. Otherwise we claim that $\partial B_r(0) \subseteq \cl{G}$. Let $z \in \partial B_r(0)$, and set $z_n \coloneqq r_n z / r$ for $n \in \N$. Now $\vert z_n \vert = r_n$, thus $z_n \in \partial B_{r_n}(0) \subseteq \cl{G}$ and therefore $(z_n)_{n \in \N}$ is a sequence in $\overline{G}$. Clearly $z_n \to z$ and since $\cl{G}$ is closed we therefore have $z \in \overline{G}$. Since $z$ was arbitrary, this concludes the claim and we have $r \in S_0$.

    Thus $S_0$ is compact and therefore has a maximum $m \geq t > 0$, which is also a maximum of $S$.
\end{remark}
...
\begin{definition}
    Let $f \in H(\D) \cap C(\cl{\D}; \C)$ be non-constant with $f(0) = 0$, then we define
    \begin{equation}
        r_f \coloneqq \max \{ r > 0 : \partial B_r(0) \subseteq f(\cl{\D}) \} > 0.
    \end{equation}
\end{definition}

\begin{theorem}[Bohr] \label{thm:bohr}
    Fix $0 < \theta < 1$ and let $f \in H(\D) \cap C(\cl{\D}; \C)$ such that $f(0) = 0$ and $M_f(\theta) = 1$. Then $r_f \geq C_\theta$, where $C_\theta > 0$ is a constant depending only on $\theta$.
\end{theorem}
...
\begin{proof}
    ...
    $$ r_f \geq \frac{1}{1 + 3 \psi(\theta, 1)} \eqqcolon C_\theta. $$
\end{proof}
...
\begin{theorem}[Thron] \label{thm:thron}
    Let $g \in H(\C)$ be transcendental, of finite order and suppose that $g$ assumes some value $w \in \C$ only finitely often. Then there exists no $f \in H(\C)$ with $f \circ f = g$.
\end{theorem}
...
\begin{proof}
    ...
    Note that since pre-images of singletons are disjoint, the union above is actually disjoint. By definition, for each $z' \in Z'$ there is some $z \in Z$ with $z' \in f^{-1}(\{ z \})$, thus
    $$ g(z') = f(f(z')) = f(z) = w. $$
    Our hypothesis on $g$ implies that $Z'$ must be finite. Therefore, all points in $Z$ are only assumed finitely often by $f$. But by \Cref{cor:transcendental-every-value-inf}, $f$ assumes all values in the complex plane infinitely often, with at most one exception; therefore $\vert Z \vert \leq 1$.
